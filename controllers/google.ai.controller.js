...
export const aiGemini = async (req, res) => {
    try {
        ...
        return res.status(200).json({ status: 'success', data: result.response.text(), file })
    } catch (error) {
        return res.status(401).json({ status: 'fail', err: error })
    }
}
...
export const aiGeminiFile = async (req, res) => {
            ...
            req.file ?
                {
                    ...
                    parts: [
                        ...
                        { text: "read this file and choose the correct answer and return the correct answer only: " + text },
                    ],
    ...
    return res.status(200).json({ status: "success", data: result.response.text() });
};
